...
namespace API.Controllers
{
    ...
    public class AgendaController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("/api/v1/agenda/obter-titulo")]

        public async Task<IActionResult> ObterTitulo([FromQuery] string titulo)
        {
            try
            {
                var todasAgendas = await _agendaServico.ObterTitulo(titulo);
                if(todasAgendas.Count == 0)
                {
                    return Ok(new ResultadoViewModel
                    {
                        Mensagem = "Nenhuma agenda Encontrado com o titulo informado",
                        Sucesso = true,
                        Data = todasAgendas
                    }); ;
                }
